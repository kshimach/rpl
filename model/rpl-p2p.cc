...
RplRoutingProtocol::ShouldRefuseP2pRdo(const RplDioHeader& dio, Ipv6Address from) const
{
    ...
    // RFC 6997 section 6.1 fixes five fields of a P2P mode DIO and then says
    // what to do when they are wrong: "A received P2P mode DIO MUST be
    ...
    // Base object." MOP is already what selects this path, so the other four
    // are checked here. Nothing generated by this module violates them --
    // the point is a peer that does, whose values JoinDodag() would
    // otherwise adopt verbatim and propagate. Version Number is the one that
    // bites hardest: HandleDio()'s lollipop comparison would treat a nonzero
    // one as a new DODAG Version and run the leave-and-rejoin migration on a
    // temporary DAG, resetting p2p state -- including the stopped flag that
    // ends the discovery -- on every such DIO.
    if (dio.GetVersionNumber() != 0)
    {
        NS_LOG_LOGIC("Refusing a P2P mode DIO whose Version Number is "
                    << +dio.GetVersionNumber() << ", not zero (RFC 6997 section 6.1)");
        return true;
    }
    if (!dio.GetGrounded())
    {
        NS_LOG_LOGIC("Refusing a P2P mode DIO with the Grounded flag clear");
        return true;
    }
    if (dio.GetPreference() != 0)
    {
        NS_LOG_LOGIC("Refusing a P2P mode DIO whose DODAGPreference is "
                    << +dio.GetPreference() << ", not zero");
        return true;
    }
    // "RPLInstanceID MUST be a local value as described in Section 5.1 of
    // [RFC6550]" -- the top bit is what says so.
    if ((dio.GetInstanceId() & RPL_LOCAL_INSTANCE_FLAG) == 0)
    {
        NS_LOG_LOGIC("Refusing a P2P mode DIO carrying the global RPLInstanceID "
                    << +dio.GetInstanceId());
        return true;
    }
    // Section 6.1 again, on the DODAG Configuration Option: "the Origin MUST
    // set the Authentication Enabled (A) flag to zero. A received P2P mode
    // DIO MUST be discarded if the A flag inside the DODAG Configuration
    // Option is not zero."
    if (dio.HasDagConfiguration() && dio.GetDagConfAuthEnabled())
    {
        NS_LOG_LOGIC("Refusing a P2P mode DIO whose DODAG Configuration sets the A flag");
        return true;
    }
