...
## 97. 角4修正群のPhase 5実測 — §95のAODV ID割当だけが制御コストを持つ

§91〜§96の修正後、単体試験(162件)を越えたシナリオ回帰として
`rpl-large-scale-system-test`で修正前後を比較した。試験では拾えない
実害(パケットロス率の悪化・制御オーバーヘッドの増大)の有無を見るため。

条件: `--scenario=3`(AODV)および`--scenario=2`(P2P)、`--nNodes=25`、
`--simTime=300`、`--RngRun=1,2,3`。同一シードで3点のコミットを比較した。

### 97.1 AODV(scenario 3)

| コミット | Data Rx (seed 1/2/3) | 平均 | 制御バイト平均 |
|---|---|---|---|
| `5a1e5e7` 修正前 | 46 / 40 / 43 | 43.0 | 192,484 |
| `321d7e2` §91〜§94まで | 52 / 40 / 47 | **46.3** | 196,566 (+2.1%) |
| `c1e27be` §95・§96込み | 52 / 40 / 47 | 46.3 | **237,841 (+23.6%)** |

読み取れること:

- **配送改善(43.0 → 46.3、+7.7%)はすべて§91〜§94由来**で、制御コストは
  ほぼゼロ(+2.1%)。RREPの重複判定・Dest SeqNo・`mayMove`・Version移行の
  4件が、これまで静かに落ちていたRREPを実際に通すようになった効果
- **制御オーバーヘッドの増加(+21%)はすべて§95由来**で、このシナリオでは
  配送の追加改善を**伴わない**
- 探索の成否指標(Discovery Attempts 4、reached 2、succeeded 2)は
  3コミットすべてで同一

§95が制御を増やす機序: 従来は連続する探索が同じinstanceIdを再利用し、
中継はrejoin barでそれを**拒否**していたため静かだった。修正後は各探索が
別IDを取るので中継が実際に加入してTrickleを回す。一時DODAG census でも
RREQインスタンス数が3→6に倍増し、各々の生存が65秒→23秒(本来の'L'=16秒
に近い値)に縮んでいる — 従来の「65秒生きるインスタンス」は、複数の探索が
1つのIDに畳み込まれて見えていた姿だった。

### 97.2 P2P(scenario 2)

3シードとも**修正前後で完全に同一**(バイト単位)。§96はこのシナリオの
挙動に一切影響しない — 探索の間隔とTargetの組み合わせがRFC 6997 §6.1の
窓に掛からないため。

### 97.3 判断が要る点(未決)

§95はRFC 9854 §4.1が中継側に**課している**拒否義務と整合を取るための
修正であり、verifierの狙い撃ちプローブでは約884秒の探索全滅窓を実測して
いる。一方で本シナリオでは、その窓に掛かる探索が無いまま**コストだけが
+21%計上**されている。

代替案として「同じTargetへの再探索に限ってバーを見る」は**採れない** —
...
したがって選択肢は「正しさを取って+21%を受け入れる」か「掃引ハーネスで
より広い動作点を測ってから判断する」かになる。3シード×25ノードの1動作点
だけで既発表の数値を更新するには薄いため、**本節は測定事実の記録に留め、
既定の変更や数値の差し替えは行っていない**。

なお§91〜§94は配送を改善して制御コストをほぼ持たないため、この論点とは
独立に採用してよい。
